...
public class DraggablePart : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        ...
        if (cameraController == null)
        {
            Debug.LogError("CameraController is null in OnBeginDrag. Attempting to find it again.");
            FindCameraController();
            if (cameraController == null)
            {
                Debug.LogError("Failed to find CameraController. Drag operation cancelled.");
                return;
            }
        }
